...
class DraggableTabs extends Component {
    ...
    handleAddArrow = () => {
        const newArrow = { id: uuidv4(), from: null, to: null, startX: 200, startY: 200, endX: 300, endY: 300 };
    ...
    getTabUnderCoords = ({ x, y }) => {
        ...
        return tabPositions.find(pos => {
            const size = tabSizes[pos.id] || { width: 100, height: 50 }; // Дефолтные значения, если размеры не установлены
            return (
    ...
    getNearestPointOnTab = (tab, coords) => {
        const { x, y } = coords;
        const tabRef = this.tabRefs[tab.id];
        if (!tabRef) return coords; // Если ссылка не найдена, возвращаем исходные координаты

        const rect = tabRef.getBoundingClientRect();
        const { width, height } = rect;

        const midPoints = [
            { x: tab.x + width / 2, y: tab.y }, // top
            { x: tab.x + width / 2, y: tab.y + height-1 }, // bottom
            { x: tab.x, y: tab.y + height / 2 }, // left
            { x: tab.x + width-1, y: tab.y + height / 2 } // right
        ];
